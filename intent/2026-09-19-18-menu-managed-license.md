...
nixarchy ships this panel on by default (olafkfreund/nixarchy#804) and declares its Omarchy
...
- `menu.py register` runs every time the panel loads (`ActionsPanel.qml:187`),
  and it only refuses when `omarchy-menu.jsonc` is a symlink or lives under
  `/nix/store` (`menu.py:95-99`). nixarchy seeds that file as a writable copy,
